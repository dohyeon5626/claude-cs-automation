...
기본 구조는 **한 프로젝트당 하나의 DB**를 가정하며, **MySQL · PostgreSQL · Oracle** 을 지원합니다 — 레포를 받아 `config.yml`만 채우면 바로 시작할 수 있고, 회사에서 일할 때 자기 PC에 켜 두기만 하면 됩니다.<br/>
...
- DB 접근          (DB가 있는 서비스에 한함) 서비스별 계정으로 접속 가능
                   MySQL · PostgreSQL · Oracle — 엔진별 드라이버는 별도 설치 (아래 "실행" 참고)
