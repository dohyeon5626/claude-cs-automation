    ...
    def _build_initial_prompt(
        ...
        if live_schema is None:
            ...
            action_section = (
                ...
                "  - 그 다음 줄부터: 한국어 Markdown 답변 (표, 요약 문장 등 활용)\n\n"
                "## 큰 결과는 다운로드 파일로 제공\n"
                "30행이 넘거나 여러 표를 함께 전달해야 하는 경우, 화면에 펼치지 말고 "
                "아래 두 블록 중 적절한 형식으로 답변하세요. 서버가 자동으로 엑셀에서 "
                "바로 열리는 다운로드 링크로 변환합니다.\n\n"
                "▸ **단일 표** → ```csv 블록\n"
                "  - 첫 줄: 헤더 (쉼표 구분)\n"
                "  - 둘째 줄부터: 데이터 (값에 쉼표·줄바꿈·큰따옴표가 있으면 큰따옴표로 감싸고 \"\"로 이스케이프)\n"
                "  - 파일명 예: ```csv 월별주문.csv\n\n"
                "▸ **여러 표를 한 파일에** → ```xlsx 블록 (다중 시트 .xlsx)\n"
                "  - 각 시트는 `## sheet: 시트이름` 줄로 구분, 그 다음 줄부터 CSV 형식\n"
                "  - 시트 이름은 31자 이하, `\\ / ? * [ ] :` 불가 (서버가 자동으로 `_`로 치환)\n"
                "  - 파일명 예: ```xlsx 보고서.xlsx\n"
                "  - 예시:\n"
                "    ```xlsx 보고서.xlsx\n"
                "    ## sheet: 월별주문\n"
